...
> 更新时间：2026-04-02 15:10:00 +0800  
> 目的：用 `attention-only` 多进程脚本，在尽量复用真实 `CPUWorker.init_device()` / `init_cpu_threads_env()` 绑核口径的前提下，观察当前 `cpu_attention_with_kv_cache` 路径在 `TP` 多进程环境下的 `L3/CCX` 局部性指标。  
...
- 这份脚本直接走 `cpu_attn_reshape_and_cache -> cpu_attn_get_scheduler_metadata -> cpu_attention_with_kv_cache`，并在每个 rank 内调用 `torch.ops._C_utils.init_cpu_threads_env(...)` 绑定线程与内存。证据：`benchmarks/kernels/cpu/benchmark_cpu_attn_mp.py`、`benchmarks/kernels/cpu/benchmark_cpu_attn.py`、`csrc/cpu/utils.cpp`。
...
- 它不加载真实模型权重，不复现真实 token 数据值，也不直接识别 `NPS`。它只能识别当前机器在运行时暴露出来的 `CPU/CORE/NODE` 拓扑。`NPS1_TP2 / NPS2_TP4 / NPS4_TP8` 在这份文档里只是实验标签，必须由外部机器配置保证。
...
3. 正式 `AMDuProfPcm` 采样时，不再用固定 `--iters` 去“估算 110 秒”；统一改用 `--min-runtime-s`，让 benchmark 在 warmup 之后持续运行到目标 steady-state 时长。
...
7. 用 `AMDuProfPcm profile -m ipc,l3,dc` 先采第一轮，因为这轮最直接对应：
   - `L3 Miss %`
   - `L3 Miss (pti)`
   - `Ave L3 Miss Latency`
   - `L3 Miss Latency From another CCX in same node (%)`
   - `Remote DRAM Reads %`
8. `summary.json` 以 dry-run 为主；正式 PCM run 的主要目标是拿 profiler 报告，不再把“是否同时拿到 benchmark summary”作为主成功判据。
9. 完成 `NPS1_TP2 / NPS2_TP4 / NPS4_TP8` 的 `global-fixed` 后，再补 `per-rank-fixed`。
10. 写结论时，`global-fixed` 和 `per-rank-fixed` 必须分开解释，不能把收益直接写成 locality 改善。
...
RESULT_ROOT=/home/zjj/vllm/test_results/P2_AttnOnly/Qwen3-30B-A3B
...
### 4.3 第一轮：`global-fixed`

下面这组命令里：

- `32/4` 表示 `Qwen3-30B-A3B` 的全局总 head 数
- `16/1/1024` 或 `16/1024/1024` 分别表示 `batch_size/q_len/kv_len`
- `exp_tag` 只是结果目录标签，不是脚本自动识别出来的 `NPS`
- 需要在对应的 `NPS` 机器配置下分别执行

#### `NPS1_TP2`

```bash
dry_run_attn_only NPS1_TP2 2 decode-like global-fixed 16 1 256 32 4
dry_run_attn_only NPS1_TP2 2 prefill-like global-fixed 16 128 128 32 4

run_attn_only_pcm NPS1_TP2 2 decode-like global-fixed 16 1 128 32 4 20000 100 150 200
run_attn_only_pcm NPS1_TP2 2 prefill-like global-fixed 16 128 128 32 4 20000 100 150 200
```

#### `NPS2_TP4`

```bash
dry_run_attn_only NPS2_TP4 4 decode-like global-fixed 16 1 1024 32 4
dry_run_attn_only NPS2_TP4 4 prefill-like global-fixed 16 1024 1024 32 4

run_attn_only_pcm NPS2_TP4 4 decode-like global-fixed 16 1 1024 32 4 10000 120 130 100
run_attn_only_pcm NPS2_TP4 4 prefill-like global-fixed 16 1024 1024 32 4 10000 120 130 100
```

#### `NPS4_TP8`

```bash
dry_run_attn_only NPS4_TP8 8 decode-like global-fixed 16 1 1024 32 4
dry_run_attn_only NPS4_TP8 8 prefill-like global-fixed 16 1024 1024 32 4

run_attn_only_pcm NPS4_TP8 8 decode-like global-fixed 16 1 1024 32 4 10000 120 130 100
run_attn_only_pcm NPS4_TP8 8 prefill-like global-fixed 16 1024 1024 32 4 10000 120 130 100
```

### 4.4 第二轮：`per-rank-fixed`
...
```bash
dry_run_attn_only NPS1_TP2 2 decode-like per-rank-fixed 16 1 1024 16 2
dry_run_attn_only NPS2_TP4 4 decode-like per-rank-fixed 16 1 1024 16 2
dry_run_attn_only NPS4_TP8 8 decode-like per-rank-fixed 16 1 1024 16 2

run_attn_only_pcm NPS1_TP2 2 decode-like per-rank-fixed 16 1 1024 16 2 10000 120 130 100
run_attn_only_pcm NPS2_TP4 4 decode-like per-rank-fixed 16 1 1024 16 2 10000 120 130 100
run_attn_only_pcm NPS4_TP8 8 decode-like per-rank-fixed 16 1 1024 16 2 10000 120 130 100
```

prefill-like 对照：

```bash
dry_run_attn_only NPS1_TP2 2 prefill-like per-rank-fixed 16 1024 1024 16 2
dry_run_attn_only NPS2_TP4 4 prefill-like per-rank-fixed 16 1024 1024 16 2
dry_run_attn_only NPS4_TP8 8 prefill-like per-rank-fixed 16 1024 1024 16 2

run_attn_only_pcm NPS1_TP2 2 prefill-like per-rank-fixed 16 1024 1024 16 2 10000 120 130 100
run_attn_only_pcm NPS2_TP4 4 prefill-like per-rank-fixed 16 1024 1024 16 2 10000 120 130 100
run_attn_only_pcm NPS4_TP8 8 prefill-like per-rank-fixed 16 1024 1024 16 2 10000 120 130 100
```
