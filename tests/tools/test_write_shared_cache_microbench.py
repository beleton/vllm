...
SRC = ROOT / "tools" / "microbench" / "write_shared_cache.c"
BIN = ROOT / "tools" / "microbench" / "write_shared_cache"
...
def test_write_shared_cache_runs_and_reports_summary() -> None:
    _compile_binary()
    ...
    assert summary["mode"] == "write-shared"
    ...
    assert summary["final_checksum"] > 0


def test_write_shared_cache_rejects_cpu_count_mismatch() -> None:
    _compile_binary()
    cpus = _pick_test_cpus(2)
