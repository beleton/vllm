...
static void* run_reader(void* arg) {
  ...
  for (uint64_t iter = 0; iter < ctx->iters; ++iter) {
    for (uint64_t line = 0; line < ctx->line_count; ++line) {
      checksum += ctx->shared_words[line * (kCacheLineBytes / sizeof(uint64_t))];
      ++touches;
    }
  }
...
int main(int argc, char** argv) {
  ...
  for (uint64_t line = 0; line < line_count; ++line) {
    shared_words[line * (kCacheLineBytes / sizeof(uint64_t))] = line + 1;
  }
  ...
  for (int i = 0; i < threads; ++i) {
    ...
    if (pthread_create(&thread_handles[i], NULL, run_reader, &thread_ctx[i]) !=
        0) {
  ...
  for (int i = 0; i < threads; ++i) {
    ...
    checksum += thread_ctx[i].checksum;
  }

  if (exit_code == 0) {
    printf("{\"mode\":\"read-shared\",\"threads\":%d,\"cpus\":[", threads);
    for (int i = 0; i < threads; ++i) {
      if (i != 0) {
        printf(",");
      }
    ...
    printf("],\"workset_bytes\":%llu,\"iters\":%llu,\"line_count\":%llu,"
           "\"elapsed_sec\":%.9f,\"total_touches\":%llu,\"checksum\":%llu}\n",
